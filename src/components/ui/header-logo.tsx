...
// ── SVG path data (frame + VIRTUO letters only, no VIDEO text) ──
...
export function HeaderLogo({ className, dark = false }: { className?: string; dark?: boolean }) {
    const base = dark ? '#0a0a0a' : '#ffffff';

    return (
        <svg viewBox='0 0 5389 5389' className={className ?? 'h-10 w-10'} aria-label='Virtuo Video logo'>
            <g transform='translate(0,5389) scale(0.1,-0.1)' stroke='none'>
                {/* Frame */}
                <path fill={base} d={FRAME} />
                ...
                {LETTERS.map((letter, i) => {
                    ...
                    return <path key={letter.id} d={letter.d} fill={base} />;
                })}
            </g>
        </svg>
    );
